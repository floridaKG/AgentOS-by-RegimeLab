...
echo ""
echo "--- Scrub Checks ---"
check "no owner username" bash -c "cd \"$STAGE\" && ! rg -qi \"${OWNER_USERNAME:-testuser}\" . --hidden --no-ignore -g '!droid-wiki/**' -g '!tests/smoke/cold_boot.sh' -g '!.ossbuild/**' -g '!tests/**' -g '!PRIVACY_BOUNDARY.md' -g '!scripts/gate-privacy.sh' -g '!scripts/gate-release.sh'"
check "no owner Linux paths" bash -c "cd \"$STAGE\" && ! rg -q \"/home/${OWNER_USERNAME:-testuser}(?:/|\\\\b)\" . --hidden --no-ignore -g '!droid-wiki/**' -g '!tests/smoke/cold_boot.sh' -g '!.ossbuild/**' -g '!tests/**' -g '!PRIVACY_BOUNDARY.md'"
check "no owner Windows-mount paths" bash -c "cd \"$STAGE\" && ! rg -q \"/mnt/c/Users/${OWNER_USERNAME:-testuser}(?:/|\\\\b)\" . --hidden --no-ignore -g '!droid-wiki/**' -g '!tests/smoke/cold_boot.sh' -g '!.ossbuild/**' -g '!tests/**' -g '!PRIVACY_BOUNDARY.md'"
check "no owner service identifiers" bash -c "cd \"$STAGE\" && ! rg -qi 'floridakg|regime-lab\\.com|hwymwmhshzmhkewusdec' . --hidden --no-ignore -g '!droid-wiki/**' -g '!tests/smoke/cold_boot.sh' -g '!.ossbuild/**' -g '!tests/**' -g '!PRIVACY_BOUNDARY.md' -g '!README.md' -g '!SETUP.md' -g '!scripts/gate-privacy.sh' -g '!scripts/gate-release.sh'"
...
check "no deferred Hindsight dependency" bash -c "! grep -Eq '^hindsight-client([<=>]|$)' \"$STAGE/requirements.txt\""
