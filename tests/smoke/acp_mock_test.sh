...
export AGENT_OS_ACP_ROOT="$CLEAN_HOME/.local/state/agent-os/acp"
...
check "model aliases include extensible providers" bash -c "grep -qE 'alias\\.(droid|pi|hermes|cline|omp|cursor|grok)' '$STAGE/.config/agent-workflows/model_aliases.toml'"
...
check "Envelope file exists" test -f "$AGENT_OS_ACP_ROOT/runs/$RUN_ID/envelope.json"

STATE=$(python3 -c "
import json
env = json.load(open('$AGENT_OS_ACP_ROOT/runs/$RUN_ID/envelope.json'))
print(env.get('state',''))
" 2>/dev/null || echo "")
...
# ── Step 7: Verify registered and extensible agents ──
echo ""
...
for agent in droid pi hermes cline omp cursor grok; do
  check "agents.yaml has $agent" bash -c "grep -q \"id: $agent\" '$AGENTS_YAML'"
done
...
# ── Step 8: Verify extensible provider routing ──
echo ""
echo "--- Step 8: ACP extensibility audit ---"
check "send.py validates against registry" bash -c "grep -q '_load_agent_ids' '$STAGE/acp/send.py'"
check "run.sh accepts safe custom providers" bash -c "grep -q 'a-zA-Z0-9._-' '$STAGE/.config/agent-workflows/lib/run.sh'"
check "acpx dispatch accepts safe custom agents" bash -c "grep -q 'a-zA-Z0-9._-' '$STAGE/.config/agent-workflows/lib/acpx-dispatch.sh'"
